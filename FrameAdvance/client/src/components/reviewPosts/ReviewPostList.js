...
const ReviewPostList = () => {
  ...
  if (filteredGameId === "0") {
    ...
    postsToDisplay = reviewPosts.filter(
      (post) => post.gameId === parseInt(filteredGameId)
    );
  }

  if (filteredCharacterId !== "0" && filteredGameId !== "0") {
    postsToDisplay = reviewPosts.filter((post) =>
      post.reviewPostCharacters.some(
        (char) => char.characterId === parseInt(filteredCharacterId)
      )
    );
  }
  ...
  return (
    <>
      ...
      <FilterReviewPosts
        ...
        setFilteredGameId={setFilteredGameId}
